	...
	<view class="order-container" :class="{ 'safe-area': isIphoneX }">
		<!-- 		<view class="order-money">
			<view class="order-cell textarea-box">
				...
				<view class="box text-overflow " @click="selectStore()">
					<text
						v-if="orderPaymentData.store_info.store_name">{{orderPaymentData.store_info.store_name}}</text>
					<text class="color-sub" v-else>请选择绑定的门店</text>
				</view>
				<text class="iconfont iconright"></text>
			</view>
		</view> -->
		<!-- 门店信息 -->
		<view class="store-box" style="margin-top: 20rpx;border-radius: 10rpx;">
			<block>
				<view class="store-info" @click="selectStore()">
					<view class="store-address-info" v-if="orderPaymentData.store_info">
						<view class="info-wrap">
							<view class="title">
								<text>{{ orderPaymentData.store_info.store_name }}</text>
							</view>
							<view class="store-detail">
								<view v-if="orderPaymentData.store_info.open_date">
									营业时间：{{ orderPaymentData.store_info.open_date }}</view>
								<view class="address">
									{{ $util.removeComma(orderPaymentData.store_info.address)||'请选择门店' }}
								</view>
							</view>
						</view>
		...
		<view class="store-box">
			<view @click="openServicePeople" class="store-info">
				...
				<view class="store-address-info">
					<view class="info-wrap">
						<view class="title">
							<text v-if="orderPaymentData.worker_id">姓名：{{ orderPaymentData.worker_name }}</text>
							<text v-if="!orderPaymentData.worker_id">请选择服务人员</text>
						</view>
						<!-- 	<view class="store-detail">
		...
		<view class="site-wrap">
			<view class="site-body" v-if="orderPaymentData">
				<!-- 	<view class="card-head">
					<image
						:src="$util.img(orderPaymentData.card_right_type == 'balance' ? 'public/uniapp/giftcard/order-icon-recharge.png' : 'public/uniapp/giftcard/order-icon-gift.png')"
						mode="widthFix"></image>
				...
				<view class="buy-num-box">
					<view class="order-cell">
						...
						<view class="box">
							<uni-number-box :min="min" :value="orderPaymentData.num" size="small"
								:modifyFlag="modifyFlag" @change="cartNumChange($event)" />
						</view>
					</view>
				</view>
		...
		<!-- 选择支付方式弹窗 -->
		<ns-payment ref="choosePaymentPopup" @confirm="orderPay" @close="payClose"
			:payMoney="orderPaymentData.pay_money"></ns-payment>
