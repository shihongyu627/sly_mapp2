					...
					<view class="order-list">
						<view class="order-item" v-for="(orderItem, orderIndex) in orderList" :key="orderIndex">
							<view class="order-header">
								...
								<text class="status-name " style="color: #FA5D34;">￥{{ orderItem.luodi_money }}</text>
							</view>
							...
							<view class="order-service">
								<image class="service-img"
									:src="$util.loadimg(orderItem&&orderItem.dealer_headimg)"
									mode="aspectFill" :lazy-load="true"></image>
								<view class="service-name">
									{{orderItem&&orderItem.dealer_name}}
								</view>
								<view class="service-tel">
									{{orderItem&&orderItem.dealer_mobile}}
								</view>
							</view>
							<view class="footerBox">
								<view class="footerBox-title">
									提货地址：
								</view>
								<view class="footerBox-content">
									{{ $util.removeComma(orderItem.full_address) }} {{ orderItem.address }}
								</view>
							</view>
							<view class="footerBox">
								<view class="footerBox-title">
									提货时间：{{ $util.timeStampTurnTime(orderItem.pass_time) }}
								</view>
							</view>
						</view>
					</view>
		...
		methods: {
			getListData(mescroll) {
				this.$api.sendRequest({
					url: '/dealer/api/luodi/orderPage',
					data: {
						page: mescroll.num,
