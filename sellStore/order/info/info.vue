<template>
	...
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
		...
		<!-- 地址信息 -->
		<view class="address-wrap" v-if="orderData.delivery_type=='express'">
			<view class="icon">
			...
			<view class="address-info">
				...
				<view class="detail">
					<text
						class="font-size-base">收货地址：{{ $util.removeComma(orderData.full_address)}}{{ orderData.address }}</text>
				</view>
			</view>
		</view>

		<!-- 地址信息 -->
		<view class="address-wrap" v-if="orderData.delivery_type=='store'">
			<view class="icon">
			...
			<view class="address-info">
				...
				<view class="detail">
					...
					<img class='store-tel' v-if="orderData.mobile" src="@/common/img/store_tel.png" mode="aspectFill"
						@click="makePhone(orderData)" />
				</view>
			</view>
		</view>
		...
		<!-- 	<view class="order-cell" v-if="orderData.goods_money>0">
				<text class="tit">修改后金额</text>
				<view class="box align-right">
					<text class="color-title">
						<text class="font-size-goods-tag">￥</text>
						{{ orderData.goods_money }}
			...
			<view class="order-cell" v-if="orderData.delivery_money>0">
			...
			<view class="order-cell" v-if="orderData.adjust_money != 0">
				...
				<view class="box align-right">
					<text class="color-base-text">
						...
						<text class="font-size-goods-tag">￥</text>
						<text>{{ orderData.adjust_money || 0 }}</text>
					</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getOrderData() {
				this.$api.sendRequest({
					url: '/dealer/api/storeOrder/detail',
					data: {
					...
					success: res => {
						...
						if (res.code >= 0) {
							...
							setTimeout(() => {
								this.$util.redirectTo('/sellStore/order/list/list');
							}, 1500);
						}
					},
					fail: res => {
