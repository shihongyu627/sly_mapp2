<template>
	...
	<view class="order-container" :data-theme="themeStyle">
						...
						<view class="order-item" v-for="(orderItem, orderIndex) in orderList" :key="orderIndex">
							...
							<view class="order-footer">
								<view class="order-action">
									<view class="order-box-btn order-box-btn1 color-base-border color-base-text"
										v-if="orderItem.order_status=='topay'"
										@click="operation('orderClose',orderItem)">取消订单
									</view>
									<view class="order-box-btn color-base-border color-base-text"
										v-if="orderItem.order_type=='1'&&orderItem.order_status=='topay'" @click="operation('orderPay',orderItem)">
										待支付
									</view>
									<view class="order-box-btn color-base-border color-base-text"
		...
		<!-- 选择支付方式弹窗 -->
		<ns-payment ref="choosePaymentPopup" @close="payClose" @confirm="orderPay(orderData)" :payMoney="payMoney">
		</ns-payment>
	...
	export default {
		...
		onShow() {
			...
			if (uni.getStorageSync('token')) {
				...
				// 	back: '/pages_tool/member/card/order/list/list?status=' + this.orderStatus
				// });
			}
		},
		methods: {
			...
			operation(action, orderData) {
				...
				switch (action) {
					...
					case 'orderClose': //关闭
						...
						break;
					case 'info': //查看
						this.$util.redirectTo('/pages_tool/member/card/order/info/info', {
