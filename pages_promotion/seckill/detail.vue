		...
		<goods-detail-view :goodsSkuDetail="goodsSkuDetail" ref="goodsDetailView">
			...
			<template v-slot:price>
				<view v-if="goodsSkuDetail.discountTimeMachine" class="goods-promotion">
					...
					<view class="price-info">
						<view class="icon-box">
							<text class="iconfont iconshangpinxiangqing-naozhong-miaosha-mian"></text>
						</view>
						<view class="price-box">
							<view class="seckill-text">限时秒杀</view>
							<view class="sale-num" v-if="goodsSkuDetail.sale_show">
								已抢{{ goodsSkuDetail.sale_num }}{{ goodsSkuDetail.unit }}</view>
						</view>
					</view>
			...
			<template v-slot:business>
				...
				<ns-goods-sku v-if="goodsSkuDetail.goods_id" ref="goodsSku" @refresh="refreshGoodsSkuDetail"
					...
					:max-buy="goodsSkuDetail.max_buy"></ns-goods-sku>
			</template>
			...
			<template v-slot:action>
				...
				<ns-goods-action :safeArea="isIphoneX">
					...
					<!-- 	<ns-goods-action-icon text="购物车" icon="icongouwuche2"
							:cornerMarkBg="themeStyle.goods_detail.goods_cart_num_corner"
							:corner-mark="cartCount > 0 ? cartCount + '' : ''" @click="goCart" /> -->
						<ns-goods-action-button class="goods-action-button " :textColor="themeStyle.btn_text_color"
							:backgroundColor="themeStyle.goods_detail.goods_btn_color" text="立即抢购" @click="seckill" />
					</template>
					<template v-else>
