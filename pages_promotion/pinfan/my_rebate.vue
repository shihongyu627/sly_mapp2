		...
		<mescroll-uni @getData="getData" top="90" ref="mescroll" :size="10" v-if="token">
			<block slot="list">
				...
				<view v-if="dataList.length == 0" style="padding-top:0">
					<ns-empty :isIndex="true" :emptyBtn ="{url: '/pages/index/index',text: '去逛逛'}" text="暂无拼团返利订单"></ns-empty>
				</view>
			</block>
		</mescroll-uni>
		...
		methods: {
			...
			toOrderDetail(id,delivery_type) {
				if(delivery_type==''){
					...
					this.$util.redirectTo('/order/detail_pickup', {order_id: id});
				}else if(delivery_type=='local'){
					this.$util.redirectTo('/order/detail_local_delivery', {order_id: id});
				}else if(delivery_type=='express'){
					this.$util.redirectTo('/order/detail', {order_id: id});
				}
			},
			//切换分类
			categoryChange(e) {
