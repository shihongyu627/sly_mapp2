			...
			<template v-slot:price>
				<view v-if="goodsSkuDetail.timeMachine" class="goods-promotion">
					<view class="price-info">
						<view class="icon-box"><text class="iconfont icontuangou"></text></view>
				...
				<view class="group-wrap padding-top">
					<view class="goods-module-wrap">
						...
						<text
							class="price price-font">{{ parseFloat(goodsSkuDetail.show_price).toFixed(2).split(".")[0] }}</text>
						<text
							class="price-symbol price-font">.{{ parseFloat(goodsSkuDetail.show_price).toFixed(2).split(".")[1] }}</text>
			...
			<template v-slot:action>
				...
				<ns-goods-action :safeArea="isIphoneX">
					<template v-if="goodsSkuDetail.goods_state == 1">
						...
						<block
							v-if="goodsSkuDetail.goods_stock < goodsSkuDetail.buy_num && !goodsSkuDetail.sku_spec_format">
							<ns-goods-action-button class="goods-action-button active1" disabled-text="库存不足"
								:disabled="true" />
						...
						<ns-goods-action-button :backgroundColor="themeStyle.goods_detail.goods_btn_color"
							:textColor="themeStyle.btn_text_color" v-else class="goods-action-button" text="立即抢购"
							@click="groupbuy" />
					</template>
					<template v-else>
						<ns-goods-action-button class="goods-action-button active2" disabled-text="该商品已下架"
							:disabled="true" />
					</template>
				</ns-goods-action>
			</template>
